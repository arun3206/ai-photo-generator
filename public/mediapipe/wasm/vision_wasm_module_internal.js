...
async function ModuleFactory(moduleArg = {}) {
  ...
  var getEmscriptenSupportedExtensions = (ctx) => {
    ...
    return (
      ctx.getSupportedExtensions()?.filter((ext) => supportedExtensions.includes(ext)) ??
      []
    );
  };
    ...
    if (program) {
      ...
      if (typeof webglLoc == "number") {
        program.uniformLocsById[location] = webglLoc = GLctx.getUniformLocation(
          program,
          program.uniformArrayNamesById[location] + (webglLoc > 0 ? `[${webglLoc}]` : ""),
        );
      }
  ...
  var _emscripten_glVertexAttribPointer = (
    ...
    if (!GLctx.currentArrayBufferBinding) {
      ...
      cb.ptr = ptr;
      cb.clientside = true;
      cb.vertexAttribPointerAdaptor = /** @this {WebGLRenderingContext} */ function (
        index,
        size,
        type,
        normalized,
  ...
  function assignWasmExports(wasmExports) {
    ...
    _interactive_segmenter_create = Module["_interactive_segmenter_create"] =
      wasmExports["Yd"];
    _interactive_segmenter_set_image = Module["_interactive_segmenter_set_image"] =
      wasmExports["Zd"];
    ...
    _mediapipeLoggerGetEncodedApiKey = Module["_mediapipeLoggerGetEncodedApiKey"] =
      wasmExports["ee"];
