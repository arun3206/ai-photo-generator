...
import type { FaceBoundingBox } from "@/features/photo-upload/types";
...
self.onmessage = async (event: MessageEvent<{ id: string; file: File }>) => {
  ...
  try {
    ...
    const allGrey = greyscale(all);
    const overallSharpness = laplacianVariance(allGrey, all.width, all.height);
    ...
    if (detections.length === 1 && detections[0]?.boundingBox) {
      ...
      const expansionX =
        raw.width * photoUploadRestrictions.face.boundingBoxExpansionRatio;
      const expansionY =
      ...
      const right = Math.min(
        bitmap.width,
        Math.ceil(raw.originX + raw.width + expansionX),
      );
      ...
      const cropGrey = greyscale(crop);
      faceSharpness = laplacianVariance(cropGrey, crop.width, crop.height);
      brightness = cropGrey.reduce((sum, value) => sum + value, 0) / cropGrey.length;
    }
    const result = classifyQuality({
      ...
      brightness,
      cropped,
      shortestSide: Math.min(bitmap.width, bitmap.height),
    });
    bitmap.close();
    ...
    self.postMessage({
      id,
      error:
        "We couldn’t check this photo on this device. Please try another photo or browser.",
    });
  }
};
